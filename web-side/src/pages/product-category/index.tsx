...
import { fetchProductCategoriesRequest, addOneProductCategoryRequest, deleteOneProductCategoryRequest, updateOneProductCategoryRequest } from './service';
...
const ProductCategoryPage: React.FC = () => {
    ...
    const [curCategory, setCurCategory] = useState<API.Product.ProductCategory | null>(null);

    // 各种网络请求
    const { runAsync: fetchProductCategories, data: productCategories } = useRequest(fetchProductCategoriesRequest);
    const { runAsync: addOneProductCategory } = useRequest(addOneProductCategoryRequest, {
        manual: true,
    })
    const { runAsync: updateOneProductCategory } = useRequest(updateOneProductCategoryRequest, {
        manual: true,
    })
    const { runAsync: deleteOneProductCategory } = useRequest(deleteOneProductCategoryRequest, {
        manual: true,
    })
                ...
                <div className={styles.addEditForm}>
                    ...
                    <Form onFinish={onFormFinished}>
                        <Form.Item name="name" label="商品类别名称">
                            <Input placeholder="商品类别名称" defaultValue={curCategory?.name} />
                        </Form.Item>
                        <Button
                            type="primary"
                            htmlType="submit"
