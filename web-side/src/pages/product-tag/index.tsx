...
import {
  fetchProductTagsRequest,
  addOneProductTagRequest,
  deleteOneProductTagRequest,
  updateOneProductTagRequest,
} from "./service";
...
const ProductTagPage: React.FC = () => {
  ...
  // 各种网络请求
  const { runAsync: fetchProductTags, data: productTags } = useRequest(
    fetchProductTagsRequest
  );
  const { runAsync: addOneProductTag } = useRequest(addOneProductTagRequest, {
    manual: true,
  });
  const { runAsync: updateOneProductTag } = useRequest(
    updateOneProductTagRequest,
    {
      manual: true,
    }
  );
  const { runAsync: deleteOneProductTag } = useRequest(
    deleteOneProductTagRequest,
    {
      manual: true,
    }
  ...
  return (
    <Card className={styles.mainCard} title="商品标签管理">
